...
action :build do
  ...
  unless ::File.exists?(aurfile) || installed
    ...
    e = execute "sudo -u #{node['jrb_workstation']['user']} tar -xf #{new_resource.name}.tar.gz" do
    ...
    em = execute "sudo -u #{node['jrb_workstation']['user']} makepkg -s --noconfirm" do
